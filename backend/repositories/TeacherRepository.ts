...
import { Teacher, CreateTeacherRequest, UpdateTeacherRequest } from '../models/Teacher';
import { DatabaseUtils } from '../utils/database';

export class TeacherRepository extends BaseRepository<Teacher, CreateTeacherRequest, UpdateTeacherRequest> {
  constructor() {
    super('teacher');
  }
  ...
  async findByTeacherId(teacher_id: string): Promise<Teacher | null> {
    ...
    const results = await DatabaseUtils.executeQuery<RowDataPacket[]>(query, [teacher_id]);
    return results.length > 0 ? this.mapRowToEntity(results[0] as any) : null;
  }

  async findByFacultyId(faculty_id: number, limit: number = 50, offset: number = 0): Promise<Teacher[]> {
    const query = `SELECT * FROM ${this.tableName} WHERE faculty_id = ? LIMIT ? OFFSET ?`;
    const results = await DatabaseUtils.executeQuery<RowDataPacket[]>(query, [faculty_id, limit, offset]);
  ...
  async findWithUserInfo(user_id: number) {
    const query = `
      ...
      WHERE t.user_id = ?
    `;
    const results = await DatabaseUtils.executeQuery<RowDataPacket[]>(query, [user_id]);
  ...
  async update(id: number, data: UpdateTeacherRequest): Promise<Teacher> {
    ...
    values.push(id);
    const query = `UPDATE ${this.tableName} SET ${fields.join(', ')} WHERE user_id = ?`;
