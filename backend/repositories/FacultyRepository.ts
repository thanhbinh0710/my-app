...
> {
  ...
  // Override findAll to use proper faculty query
  async findAll(limit: number = 10, offset: number = 0): Promise<Faculty[]> {
    const query = `SELECT * FROM ${this.tableName} ORDER BY faculty_id LIMIT ? OFFSET ?`;
    const results = await DatabaseUtils.executeQuery<RowDataPacket[]>(query, [
      limit,
      offset,
    ]);
